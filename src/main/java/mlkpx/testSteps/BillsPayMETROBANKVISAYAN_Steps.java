...
public class BillsPayMETROBANKVISAYAN_Steps extends Base_Steps{
    ...
    public void BPSendout_METROBANKVISAYAN_01()throws Exception {
        ...
        waitSleep(5000);
        click(billsPayPageObjects.ChooseOptionMETROBANKVISAYAN(), "METROBANK VISAYAN KHARIS Partner");
        waitUntilLoadingGone(10000);
        waitSleep(5000);
        type(billsPayPageObjects.BPAccNum(), "BP METROBANK VISAYAN KHARIS Account Number", "K020110");
        type(billsPayPageObjects.BPLastName(), "BP Last Name", "LAMBUSON ");
        type(billsPayPageObjects.BPFirstName(), "BP First Name", "EDYEL  ");
        ...
        if(isVisible(billsPayPageObjects.billsPay_ConfiInfo(), getText(billsPayPageObjects.billsPay_ConfiInfo()))) {
            ...
            reader.writeBillsPayMETROBANKVISAYANKptnData(kptnValues);
    ...
    public void BPSendout_METROBANKVISAYAN_02()throws Exception {
        ...
        if(isVisible(billsPayPageObjects.billsPay_ConfiInfo(), getText(billsPayPageObjects.billsPay_ConfiInfo()))) {
            ...
            String kptnText = getText(billsPayPageObjects.kptnText());
            List<String> kptnValues = Collections.singletonList(kptnText);
            reader.writeBillsPayRemoteMETROBANKVisayanKptnData(kptnValues);
            LoggingUtils.info("Bills Pay Successful" +
                    "The payment was successfully created." +
                    "Please prepare printer to print document.");
            click(billsPayPageObjects.proceedToPrinting(), "Proceed to Printing");
...
//B I L L S  P A Y    M E T R O B A N K   V I S A Y A N   C H A N G E  D E T A I L S

    public void BPChangeDetails_METROBANKVISAYAN_01()throws Exception{
        waitSleep(2000);
        scrollToElement(billsPayPageObjects.BPChangeDetails());
        click(billsPayPageObjects.BPChangeDetails(), "METROBANK VISAYAN KHARIS Partner Bills Pay Change Details");
        if(isVisible(billsPayPageObjects.BillsPayChange(), getText(billsPayPageObjects.BillsPayChange()))) {
            ...
            waitSleep(3000);
            String BillsPayKPTN = reader.getBillsPayMETROBANKVisayan1KPTN();
            type(billsPayPageObjects.RefNum(), "METROBANK VISAYAN KHARIS Partner KTPN Number",BillsPayKPTN);
    ...
    public void BPChangeDetails_METROBANKVISAYAN_02()throws Exception{
        waitSleep(2000);
        scrollToElement(billsPayPageObjects.BPChangeDetails());
        click(billsPayPageObjects.BPChangeDetails(), "Bills Pay METROBANK VISAYAN KHARIS Change Details");
        waitSleep(5000);
        if(isVisible(billsPayPageObjects.BillsPayChange(), getText(billsPayPageObjects.BillsPayChange()))) {
            LoggingUtils.info("Navigated the BillsPay Change Details page");
            waitSleep(3000);
            reader.refreshTestData();
            waitSleep(3000);
            String RemoteBillsPayKPTN = reader.getRemoteBillsPayMETROBANKVisayan1KPTN();
            type(billsPayPageObjects.RefNum(), "METROBANK VISAYAN KHARIS KTPN Number",RemoteBillsPayKPTN);
            click(billsPayPageObjects.SearchButton(), "Search Button");
        }
    ...
    public void BPCancellation_METROBANKVISAYAN_01()throws Exception{
        ...
        if(isVisible(billsPayPageObjects.BillsPayText(), getText(billsPayPageObjects.BillsPayText()))) {
            ...
            reader.refreshTestData();
            waitSleep(3000);
            String BillsPayKPTN = reader.getBillsPayMETROBANKVisayanKPTN();
            type(billsPayPageObjects.RefNum(), "METROBANK VISAYAN KHARIS Partner KTPN Number",BillsPayKPTN);
            click(billsPayPageObjects.SearchButton(), "Search Button");
        }
        if(isVisible(billsPayPageObjects.BillsPayText(), getText(billsPayPageObjects.BillsPayText()))){
            type(billsPayPageObjects.IRNum(), "METROBANK VISAYAN KHARIS KTPN Number", "2111111111262");
    ...
    public void BPCancellation_METROBANKVISAYAN_02()throws Exception{
        ...
        if(isVisible(billsPayPageObjects.BillsPayText(), getText(billsPayPageObjects.BillsPayText()))) {
            ...
            reader.refreshTestData();
            waitSleep(3000);
            String BillsPayRemoteKPTN = reader.getRemoteBillsPayMETROBANKVisayanKPTN();
            type(billsPayPageObjects.RefNum(), "METROBANK VISAYAN KHARIS Partner KTPN Number",BillsPayRemoteKPTN);
            click(billsPayPageObjects.SearchButton(), "Search Button");
        }
