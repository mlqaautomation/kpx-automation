...
public class BillsPaySANTAPHARMA_Steps extends Base_Steps{
    ...
    public void BPSendout_SANTAPHARMA_01()throws Exception {
        ...
        waitSleep(5000);
        click(billsPayPageObjects.ChooseOptionSANTAPHARMA(), "SANTAPHARMA Partner");
        waitUntilLoadingGone(10000);
        waitSleep(5000);
        type(billsPayPageObjects.BPAccNum(), "BP SANTAPHARMA Account Number", "SPC-001");
        ...
        if(isVisible(billsPayPageObjects.billsPay_ConfiInfo(), getText(billsPayPageObjects.billsPay_ConfiInfo()))) {
            ...
            reader.writeBillsPaySANTAPharmaKptnData(kptnValues);
    ...
    public void BPSendout_SANTAPHARMA_02()throws Exception {
        ...
        if(isVisible(billsPayPageObjects.billsPay_ConfiInfo(), getText(billsPayPageObjects.billsPay_ConfiInfo()))) {
            ...
            String kptnText = getText(billsPayPageObjects.kptnText());
            List<String> kptnValues = Collections.singletonList(kptnText);
            reader.writeBillsPayRemoteSANTAPharmaKptnData(kptnValues);
            LoggingUtils.info("Bills Pay Successful" +
                    "The payment was successfully created." +
                    "Please prepare printer to print document.");
            click(billsPayPageObjects.proceedToPrinting(), "Proceed to Printing");
...
//B I L L S  P A Y    S A N T A P H A R M A   C H A N G E  D E T A I L S

    public void BPChangeDetails_SANTAPHARMA_01()throws Exception{
        waitSleep(2000);
        scrollToElement(billsPayPageObjects.BPChangeDetails());
        click(billsPayPageObjects.BPChangeDetails(), "SANTAPHARMA Partner Bills Pay Change Details");
        if(isVisible(billsPayPageObjects.BillsPayChange(), getText(billsPayPageObjects.BillsPayChange()))) {
            ...
            waitSleep(3000);
            String BillsPayKPTN = reader.getBillsPaySANTAPharma1KPTN();
            type(billsPayPageObjects.RefNum(), "SANTAPHARMA Partner KTPN Number",BillsPayKPTN);
    ...
    public void BPChangeDetails_SANTAPHARMA_02()throws Exception{
        waitSleep(2000);
        scrollToElement(billsPayPageObjects.BPChangeDetails());
        click(billsPayPageObjects.BPChangeDetails(), "Bills Pay SANTAPHARMA Change Details");
        waitSleep(5000);
        if(isVisible(billsPayPageObjects.BillsPayChange(), getText(billsPayPageObjects.BillsPayChange()))) {
            LoggingUtils.info("Navigated the BillsPay Change Details page");
            waitSleep(3000);
            reader.refreshTestData();
            waitSleep(3000);
            String RemoteBillsPayKPTN = reader.getRemoteBillsPaySANTAPharma1KPTN();
            type(billsPayPageObjects.RefNum(), "SANTAPHARMA KTPN Number",RemoteBillsPayKPTN);
            click(billsPayPageObjects.SearchButton(), "Search Button");
        }
    ...
    public void BPCancellation_SANTAPHARMA_01()throws Exception{
        ...
        if(isVisible(billsPayPageObjects.BillsPayText(), getText(billsPayPageObjects.BillsPayText()))) {
            ...
            reader.refreshTestData();
            waitSleep(3000);
            String BillsPayKPTN = reader.getBillsPaySANTAPharmaKPTN();
            type(billsPayPageObjects.RefNum(), "SANTAPHARMA Partner KTPN Number",BillsPayKPTN);
            click(billsPayPageObjects.SearchButton(), "Search Button");
        }
        if(isVisible(billsPayPageObjects.BillsPayText(), getText(billsPayPageObjects.BillsPayText()))){
            type(billsPayPageObjects.IRNum(), "SANTAPHARMA KTPN Number", "2111111111262");
    ...
    public void BPCancellation_SANTAPHARMA_02()throws Exception{
        ...
        if(isVisible(billsPayPageObjects.BillsPayText(), getText(billsPayPageObjects.BillsPayText()))) {
            ...
            reader.refreshTestData();
            waitSleep(3000);
            String BillsPayRemoteKPTN = reader.getRemoteBillsPaySANTAPharmaKPTN();
            type(billsPayPageObjects.RefNum(), "SANTAPHARMA Partner KTPN Number",BillsPayRemoteKPTN);
            click(billsPayPageObjects.SearchButton(), "Search Button");
        }
