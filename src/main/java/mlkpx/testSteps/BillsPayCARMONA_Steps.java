...
public class BillsPayCARMONA_Steps extends Base_Steps{
    ...
    public void BPSendout_CARMONA_01()throws Exception {
        ...
        waitSleep(5000);
        click(billsPayPageObjects.ChooseOptionCARMONA(), "CARMONA Partner");
        waitUntilLoadingGone(10000);
        waitSleep(5000);
        type(billsPayPageObjects.BPAccNum(), "BP CARMONA Account Number", "114101358");
        ...
        if(isVisible(billsPayPageObjects.billsPay_ConfiInfo(), getText(billsPayPageObjects.billsPay_ConfiInfo()))) {
            ...
            reader.writeBillsPayCARMONAKptnData(kptnValues);
    ...
    public void BPSendout_CARMONA_02()throws Exception {
        ...
        if(isVisible(billsPayPageObjects.billsPay_ConfiInfo(), getText(billsPayPageObjects.billsPay_ConfiInfo()))) {
            ...
            String kptnText = getText(billsPayPageObjects.kptnText());
            List<String> kptnValues = Collections.singletonList(kptnText);
            reader.writeBillsPayRemoteCARMONAKptnData(kptnValues);
            LoggingUtils.info("Bills Pay Successful" +
                    "The payment was successfully created." +
                    "Please prepare printer to print document.");
            click(billsPayPageObjects.proceedToPrinting(), "Proceed to Printing");
...
//B I L L S  P A Y    C A R M O N A   C H A N G E  D E T A I L S

    public void BPChangeDetails_CARMONA_01()throws Exception{
        waitSleep(2000);
        scrollToElement(billsPayPageObjects.BPChangeDetails());
        click(billsPayPageObjects.BPChangeDetails(), "CARMONA Partner Bills Pay Change Details");
        if(isVisible(billsPayPageObjects.BillsPayChange(), getText(billsPayPageObjects.BillsPayChange()))) {
            ...
            waitSleep(3000);
            String BillsPayKPTN = reader.getBillsPayCARMONA1KPTN();
            type(billsPayPageObjects.RefNum(), "CARMONA Partner KTPN Number",BillsPayKPTN);
    ...
    public void BPChangeDetails_CARMONA_02()throws Exception{
        waitSleep(2000);
        scrollToElement(billsPayPageObjects.BPChangeDetails());
        click(billsPayPageObjects.BPChangeDetails(), "Bills Pay CARMONA Change Details");
        waitSleep(5000);
        if(isVisible(billsPayPageObjects.BillsPayChange(), getText(billsPayPageObjects.BillsPayChange()))) {
            LoggingUtils.info("Navigated the BillsPay Change Details page");
            waitSleep(3000);
            reader.refreshTestData();
            waitSleep(3000);
            String RemoteBillsPayKPTN = reader.getRemoteBillsPayCARMONA1KPTN();
            type(billsPayPageObjects.RefNum(), "CARMONA KTPN Number",RemoteBillsPayKPTN);
            click(billsPayPageObjects.SearchButton(), "Search Button");
        }
    ...
    public void BPCancellation_CARMONA_01()throws Exception{
        ...
        if(isVisible(billsPayPageObjects.BillsPayText(), getText(billsPayPageObjects.BillsPayText()))) {
            ...
            reader.refreshTestData();
            waitSleep(3000);
            String BillsPayKPTN = reader.getBillsPayCARMONAKPTN();
            type(billsPayPageObjects.RefNum(), "CARMONA Partner KTPN Number",BillsPayKPTN);
            click(billsPayPageObjects.SearchButton(), "Search Button");
        }
        if(isVisible(billsPayPageObjects.BillsPayText(), getText(billsPayPageObjects.BillsPayText()))){
            type(billsPayPageObjects.IRNum(), "CARMONA KTPN Number", "2111111111262");
    ...
    public void BPCancellation_CARMONA_02()throws Exception{
        ...
        if(isVisible(billsPayPageObjects.BillsPayText(), getText(billsPayPageObjects.BillsPayText()))) {
            ...
            reader.refreshTestData();
            waitSleep(3000);
            String BillsPayRemoteKPTN = reader.getRemoteBillsPayCARMONAKPTN();
            type(billsPayPageObjects.RefNum(), "CARMONA Partner KTPN Number",BillsPayRemoteKPTN);
            click(billsPayPageObjects.SearchButton(), "Search Button");
        }
