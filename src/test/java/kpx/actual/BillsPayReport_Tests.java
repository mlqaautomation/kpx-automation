...
public class BillsPayReport_Tests extends BaseTest {
    ...
    @Test(dependsOnMethods = "loginGoogle" , priority = 9, description = "To verify success in generating Bills pay Cancellation Report (Monthly)")
    public void  successInGeneratingBillspayCancellationReportMonthly()throws Exception{
        if(this.homeSteps.isInHomePage()){
            this.billsPayReportSteps.BP_CR_TC_04();
        }
    }
    @Test(dependsOnMethods = "loginGoogle" , priority = 9, description = "To verify success in generating Bills pay Change Details Report (Monthly)")
    public void  successInGeneratingBillspayChangeDetailsReportMonthly()throws Exception{
        if(this.homeSteps.isInHomePage()){
            this.billsPayReportSteps.BP_CR_TC_05();
        }
    }
    @Test(dependsOnMethods = "loginGoogle" , priority = 9, description = "To verify success in generating Bills pay Change Details Report (Date Range)")
    ...
    @Test(dependsOnMethods = "loginGoogle" , priority = 10, description = "To verify generating Report with no date provided ")
    public void  generatingReportwithnodateprovided()throws Exception{
        if(this.homeSteps.isInHomePage()){
            this.billsPayReportSteps.BP_CR_TC_08();
        }
    }
    @Test(dependsOnMethods = "loginGoogle" , priority = 11, description = "To verify generating Report with future date ")
    public void  generatingReportwithfuturedate()throws Exception{
        if(this.homeSteps.isInHomePage()){
            this.billsPayReportSteps.BP_CR_TC_09();
        }
    }
    @Test(dependsOnMethods = "loginGoogle" , priority = 12, description = "To verify Clear Button")
